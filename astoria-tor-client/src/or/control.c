                             ...
                             int reason_code)
{
  ...
  switch (tp)
    {
    case CIRC_EVENT_LAUNCHED: status = "LAUNCHED";
    log_debug(LD_CIRC, "[TIP] Circuit has been launched!");
    break;
    case CIRC_EVENT_BUILT: status = "BUILT";
    log_debug(LD_CIRC, "[TIP] Circuit has been built!");
    break;
    case CIRC_EVENT_EXTENDED: status = "EXTENDED";
    log_debug(LD_CIRC, "[TIP] Circuit has been extended!");
    break;
    case CIRC_EVENT_FAILED: status = "FAILED";
    log_debug(LD_CIRC, "[TIP] Circuit failure!");
    break;
    case CIRC_EVENT_CLOSED: status = "CLOSED";
    log_debug(LD_CIRC, "[TIP] Circuit has been closed!");
    break;
    default:
      log_warn(LD_BUG, "[TIP] Unrecognized status code %d", (int)tp);
      tor_fragile_assert();
      return 0;
    }
