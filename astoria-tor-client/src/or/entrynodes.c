...
/** Returns list of guards to be used as the entry guard of a circuit.
 *
...
return_live_entry_guards(cpath_build_state_t *state, int for_directory, dirinfo_type_t dirinfo_type, int *n_options_out)
{
  const or_options_t *options = get_options();
 ...
 retry:
  ...
  /* Populate the list of live entry guards so that we pick one of
     them. */
  retval = populate_live_entry_guards(live_entry_guards,entry_guards,chosen_exit,dirinfo_type,for_directory,need_uptime, need_capacity);
 ...
 choose_and_finish:
  ...
  //return node;
  return live_entry_guards;
}



const smartlist_t *
return_live_entry_list(cpath_build_state_t *state)
{
  return return_live_entry_guards(state, 0, NO_DIRINFO, NULL);
}
                           ...
                           int need_uptime, int need_capacity)
{
  ...
  log_debug(LD_CIRC, "[TIP] For directory: %d", for_directory);
  log_debug(LD_CIRC, "[TIP] Number needed: %d", num_needed);

  SMARTLIST_FOREACH_BEGIN(all_entry_guards, const entry_guard_t *, entry) {
      const char *msg;
      node = entry_is_live(entry, entry_flags, &msg);
      log_debug(LD_CIRC, "[TIP] Checking validity of node: %s as an entry node", node_describe(node));
      if (!node)
