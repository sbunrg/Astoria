...
static channel_t * channel_connect_for_circuit(const tor_addr_t *addr, uint16_t port, const char *id_digest);
...
static int as_onion_extend_cpath(origin_circuit_t *circ, extend_info_t *entry);
...
static int
as_onion_populate_cpath(origin_circuit_t *circ, extend_info_t *entry)
{
  int n_tries = 0;
  const int using_ntor = circuits_can_use_ntor();
...
#define MAX_POPULATE_ATTEMPTS 32
  ...
  while (1) {
    int r = as_onion_extend_cpath(circ, entry);
    if (r < 0) {
      log_info(LD_CIRC,"Generating cpath hop failed.");
      return -1;
    }
...
{
  log_debug(LD_CIRC, "[TIP] Initializing new circuit for purpose: %d", purpose);
  /*Disabling all cap and uptime flags here*/
  //flags = flags & ~(CIRCLAUNCH_NEED_UPTIME | CIRCLAUNCH_NEED_CAPACITY);
...
/** Pick a good entry server for the Tor AS circuit to be built according to
 ...
 */
const node_t *
choose_good_entry_server_as(uint8_t purpose, cpath_build_state_t *state)
{
  const node_t *choice;
  smartlist_t *excluded;
  const or_options_t *options = get_options();
  ...
  const node_t *node;
  log_debug(LD_CIRC, "[TIP] UseEntryGuards: %d",options->UseEntryGuards);

  if (state && options->UseEntryGuards &&
      (purpose != CIRCUIT_PURPOSE_TESTING || options->BridgeRelay)) {
    /* This request is for an entry server to use for a regular circuit,
     * and we use entry guard nodes.  Just return one of the guard nodes.  */
     choice = choose_random_entry(state);
     log_debug(LD_CIRC, "[TIP] Chosen entry node: %s", node_describe(choice));
     return choice;
  }
 ...
 /** Return the list of all nodes which support an existing connection as an exit node.
  * Similar to choose_good_server_exit_general(), except that it does not select one of them.
  */
smartlist_t *
return_all_good_exit_server_general(int need_uptime, int need_capacity)
{
  int *n_supported;
...
//         n_pending_connections);
  ...
  SMARTLIST_FOREACH_BEGIN(the_nodes, const node_t *, node)  {
    ...
    //log_debug(LD_CIRC, "[TIP] Looking if node %s supports the pending connection.", node_describe(node));
...
//             router->nickname);
  ...
  log_info(LD_CIRC, "[TIP] Found %d servers that might support %d/%d pending connections.", n_best_support, best_support >= 0 ? best_support : 0, n_pending_connections);
  ...
  else
  {
    ...
    if (best_support == -1)
    {
      ...
      {
        log_info(LD_CIRC, "We couldn't find any live%s%s routers; falling back to list of all routers.", need_capacity?", fast":"", need_uptime?", stable":"");
        tor_free(n_supported);
        return return_all_good_exit_server_general(0, 0);
      }
      log_notice(LD_CIRC, "All routers are down or won't exit%s -- choosing a doomed exit at random.", options->ExcludeExitNodesUnion_ ? " or are Excluded" : "");
    }
    ...
    for (attempt = 0; attempt < 2; attempt++)
    {
      ...
      SMARTLIST_FOREACH_BEGIN(the_nodes, const node_t *, node)      {
        if (n_supported[node_sl_idx] != -1 && (attempt || node_handles_some_port(node, needed_ports)))
        {
...
static smartlist_t *return_all_good_entry_servers_general(cpath_build_state_t *state, uint8_t purpose, int flags)
{
    smartlist_t *entry_nodes = smartlist_new();
    const or_options_t *options = get_options();
    log_debug(LD_CIRC, "[TIP] Generating list of entry nodes.");
    int number_entry = 0;
    //Check if we have to use entry guards.
    if (state && options->UseEntryGuards && (purpose != CIRCUIT_PURPOSE_TESTING || options->BridgeRelay))
    {
        //We do. So now check if we have any already stored guards. If we do, put them in a smartlist.
        entry_nodes = return_live_entry_list(state);
        number_entry = smartlist_len(entry_nodes);
        log_debug(LD_CIRC, "[TIP] Tor is configured to use entry guards. There are currently %d guards.", number_entry);
        if(number_entry > 0)
        {
            SMARTLIST_FOREACH_BEGIN(entry_nodes, node_t *, node)
            {
                log_debug(LD_CIRC, "[TIP] Node %s is an entry guard", node_describe(node));
            }SMARTLIST_FOREACH_END(node);
        }
    }
    else
    {
        //We don't need to use guards. So just create a list of nodes with different families from already selected nodes.
        //To do this, we first create a list of nodes that we want to exclude.
        log_debug(LD_CIRC, "[TIP] We set EntryGuards = 0, so Tor is not using entry guards. Creating an exclude list.");
        const node_t *node;
        ...
        if (firewall_is_fascist_or())
        {
            /* Exclude all ORs that we can't reach through our firewall */
            smartlist_t *nodes = nodelist_get_list();
            SMARTLIST_FOREACH(nodes, const node_t *, node, {if (!fascist_firewall_allows_node(node)) smartlist_add(excluded, (void*)node);});
        }
        if (state)
        {
            if (state->need_uptime)
        ...
        if (options->AllowInvalid_ & ALLOW_INVALID_ENTRY)
            flags |= CRN_ALLOW_INVALID;
        entry_nodes = return_all_but_excluded(excluded, options->ExcludeNodes, flags);
        SMARTLIST_FOREACH_BEGIN(entry_nodes, node_t *, node)
        {
            //log_debug(LD_CIRC, "[TIP] Node %s is available as an entry node.", node_describe(node));
        }SMARTLIST_FOREACH_END(node);
    }
    return entry_nodes;
}
char *parse_ip(char *description)
{
    char parsed_ip[16];
    //log_debug(LD_CIRC, "[TIP] Description: %s", description);
    char *copy = strdup(description), *token;
    int count = 0;
    char **copy_tokens = &copy;
    token = strsep(copy_tokens, " ");
    while(token)
    {
        if(count == 2)
        {
            strcpy(parsed_ip, token);
            //log_debug(LD_CIRC, "[TIP] Parsed IP: %s", parsed_ip);
            return parsed_ip;
        }
        else
        {
    ...
    log_debug(LD_CIRC, "[TIP] Error while parsing the IP. Returning NULL.");
    return NULL;
}

/**
  * Find the systems default net interface and then find its IP address
**/
char *get_own_ip()
{
    FILE *f;
    ...
    {
        log_debug(LD_CIRC,"getifaddrs ERROR");
        exit(EXIT_FAILURE);
    }
    ...
    for (ifa = ifaddr; ifa != NULL; ifa = ifa->ifa_next)
    {
        ...
        if(strcmp( ifa->ifa_name , p) == 0)
        {
            if (family == fm)
            {
                ...
                if (s != 0)
                {
                    log_debug(LD_CIRC,"getnameinfo() failed: %s", gai_strerror(s));
                    exit(EXIT_FAILURE);
                }
                log_debug(LD_CIRC,"address: %s", host);
            }
        }
    }
    freeifaddrs(ifaddr);
...
unsigned int rand_interval(unsigned int min, unsigned int max)
{
    ...
    log_debug(LD_CIRC,"[TIP] Picking a random number in the interval [%d, %d]", min, max);
    /* Create equal size buckets all in a row, then fire randomly towards
 *      * the buckets until you land in one of them. All buckets are equally
 *           * likely. If you land off the end of the line of buckets, try
 *           again. */
    do
    {
        r = rand();
    ...
    log_debug(LD_CIRC,"[TIP] Selecting: %d", min+(r/buckets));

    return min + (r / buckets);
}

void generate_distribution(double *values, int n_vals, int uniform, double *output)
{
    double total = 0;
    if(uniform == 0)
    {
        for( int i = 0 ; i < n_vals ; i ++)
            total += values[i];
        for( int i = 0 ; i < n_vals ; i ++)
            output[i] = (double)((double)values[i]/(double)total);
    }
    else
        for(int i = 0 ; i < n_vals ; i ++)
            output[i] = (double)((double)1/(double)n_vals);
    for(int i = 0 ; i < n_vals ; i ++)
        log_debug(LD_CIRC, "[TIP] Distribution[%d]: %f", i, output[i]);
}

void tune_distributions(double *d1, double *d2, int n_vals, double alpha, double *output)
{
    double denom = 0;
    for(int i =0 ; i < n_vals ; i ++)
        denom = denom + (d1[i]*alpha) + (d2[i]*(1-alpha));
    for(int i = 0 ; i < n_vals ; i ++)
    {
        output[i] = ((d1[i]*alpha)+(d2[i]*(1-alpha)))/denom;
        log_debug(LD_CIRC, "[TIP] Alpha: %f. D1: %f, D2: %f, Output: %f", alpha, d1[i], d2[i], output[i]);
    }
}

/*Selects an index from 1 to n_vals according to distribution*/
int pick_from_dist(double *distribution, int n_vals)
{
    double running_sum[n_vals+1];
    int ret_index = 0;
    running_sum[0] = 0;
    for(int i = 0 ; i < n_vals ; i ++)
    {
        running_sum[i+1] = running_sum[i] + distribution[i];
        //log_debug(LD_CIRC, "[TIP] Distribution[%d]: %f, RS: %f", i, distribution[i], running_sum[i+1]);
    }
    double random_number = ((double)rand()/(double)(RAND_MAX))*running_sum[n_vals];
    for(int i = 0 ; i < n_vals ; i ++)
    {
        if(distribution[i] == 0)
      	continue;
        if((random_number >= running_sum[i]) && (random_number < running_sum[i+1]))
        {
            ret_index = i;
            log_debug(LD_CIRC, "[TIP] Random number %f was found between index %d [RS: %f] and %d [RS: %f], [Dist: %f], [Dist: %f]", random_number, i, running_sum[i], i+1, running_sum[i+1], distribution[i], distribution[i+1]);
            break;
        }
        //else
            //log_debug(LD_CIRC, "[TIP] Random number %f was NOT found between index %d [RS: %f] and %d [RS: %f]", random_number, i, running_sum[i], i+1, running_sum[i+1]);
    }
    log_debug(LD_CIRC, "[TIP] Index chosen: %d , Distribution value: %f", ret_index, distribution[ret_index]);
    return ret_index;
}
 ...
 */
 origin_circuit_t *
circuit_establish_circuit(uint8_t purpose, extend_info_t *exit, int flags, char *dest_addr)
{
  //removing NEED_CAPACITY flag here.
  //flags = flags & ~(CIRCLAUNCH_NEED_CAPACITY|CIRCLAUNCH_NEED_UPTIME);
  origin_circuit_t *circ;
  extend_info_t *entry;
  int err_reason = 0, number_exits = 0, temp = 0;
  smartlist_t *good_exits = smartlist_new();
  char as_src[10], as_dest[10], ip_src[16], *ip_dest;

  /*These things need to be changed on each run*/
  double alpha = 1; /*Try for alpha \in {0, 0.5, 1} */
  char *exits_filename = "ES-exits-lp-1.log";
  char *attack_filename = "ES-attacks-lp-1.log";
  char *bandwidths_filename = "ES-bw-lp-1.log";
  char *bgp_time_filename = "ES-bgp-lp-1.log";
  strcpy(ip_src, "5.134.112.4");

  /*IP of sources: 130.245.150.14 (US), 186.227.192.21 (BR), 122.0.69.130 (CN), 5.56.17.2 (DE), 5.134.112.4 (ES), 62.73.3.153 (FR), 77.75.123.187 (GB), 104.167.254.2 (IR), 185.17.159.129 (IT), 95.213.137.10 (RU)*/

  FILE *attacks = fopen(attack_filename, "a");
  FILE *file = fopen(exits_filename, "a");
  FILE *bw_file = fopen(bandwidths_filename, "a");

  if (file == NULL || attacks == NULL || bw_file)
  {
    log_debug(LD_CIRC,"[TIP] Error opening file %s. Circuit info not written to disk.", exits_filename);
  }

  fprintf(file,"------------------------");
  fprintf(file, "Destination IP: %s \n", dest_addr);

  ip_dest = strdup(dest_addr);
  log_debug(LD_CIRC, "[TIP] IP of Destination: %s", ip_dest);
  if(strcmp(ip_dest,"")!=0)
    strcpy(as_dest, do_whois(ip_dest));
  else
    strcpy(as_dest, "1");
  strcpy(as_src, do_whois(ip_src));

  log_debug(LD_CIRC, "[TIP] Establishing new circuit for purpose: %d, to destination: %s (AS %s), from source: %s (AS %s)", purpose, ip_dest, as_dest, ip_src, as_src);
  if(1)
  {
    circ = origin_circuit_init(purpose, flags);
    if(strcmp(extend_info_describe(exit),"<null>")!=0)
    {
        log_debug(LD_CIRC, "[TIP] Exit not NULL. Using given exit: %s", extend_info_describe(exit));
    }
    else
    {
        good_exits = return_all_good_exit_server_general(circ->build_state->need_uptime, circ->build_state->need_capacity);
        number_exits = smartlist_len(good_exits);
        log_debug(LD_CIRC, "[TIP] Exit not provided. Temporarily selecting one on our own.");
        log_debug(LD_CIRC, "[TIP] Number of available exit nodes: %d", number_exits);
        exit = extend_info_from_node(smartlist_get(good_exits, 1),0);
        temp = 1;
    }
    ...
    if(temp)
    {
        const smartlist_t *entry_nodes = smartlist_new();
        entry_nodes = return_all_good_entry_servers_general(circ->build_state, purpose, flags);

        smartlist_t *as_entry = smartlist_new();
        smartlist_t *as_exit = smartlist_new();
        double *entry_node_bw = smartlist_return_bw_weights(entry_nodes, WEIGHT_FOR_GUARD);
        double *exit_node_bw = smartlist_return_bw_weights(good_exits, WEIGHT_FOR_EXIT);
        SMARTLIST_FOREACH_BEGIN(entry_nodes, const node_t *, node)
        {
            char ip[16], trunc_ip[11];
            char *as = malloc(sizeof(char) * 10);
            strcpy(ip, parse_ip(node_describe(node)));
            strcpy(as, do_whois(ip));
            fprintf(file, "\nentry node option: %s [%s]", ip, as);
            //if(strcmp(as,"-1")!=0)
            smartlist_add(as_entry, as);
            //else log_debug(LD_CIRC,"[TIP] Not adding entry node %s to smartlist. No AS found in local db",ip);
        } SMARTLIST_FOREACH_END(node);
        fprintf(file, "\n");
        int f = 0;
        SMARTLIST_FOREACH_BEGIN(good_exits, const node_t *, node)
        {
            char ip[16], trunc_ip[11];
            // TODO: free the list!
            char *as = malloc(sizeof(char) * 10);
            strcpy(ip, parse_ip(node_describe(node)));
            strcpy(as, do_whois(ip));
            fprintf(file, "\nexit node option: %s [%s]", ip, as);
            //if(strcmp(as,"-1")!=0)
            smartlist_add(as_exit, as);
            //else log_debug(LD_CIRC,"[TIP] Not adding exit node %s to smartlist. No AS found in local db",ip);
        } SMARTLIST_FOREACH_END(node);
        fprintf(file, "\n-------------------------------------\n");
    	fclose(file);


        /*This is where we have the entry and exit node selection logic*/
        /*We're just going to pick a random entry node and a random exit node*/

        	/*
        // MONO-EXAMPLE-TEST
        char* dests = "174 32490";
        char* pairs = "32490 174 15169 3240";
        asnset result[2];
        int res = get_aspath(dests, pairs, 2, result);
        log_debug(LD_CIRC, "[MONO]: get_aspath: %d.", res);
        */

        // For query to bgp_sim
        char dests[1000000];
        char pairs[1000000];
	...
	strcat(dests, as_src);
        strcat(dests, " ");
	strcat(dests, as_dest);
        strcat(dests, " ");

        log_debug(LD_CIRC, "[MONO]: Exits: %d.", smartlist_len(as_exit));

	// Paths from exits to destination (and vise versa)
	for (i = 0; i < smartlist_len(as_exit); ++i) {
	    //log_debug(LD_CIRC, "[MONO]: %d| %s.", i, (char*)smartlist_get(as_exit, i));
	    char *exit = (char*)smartlist_get(as_exit, i);
	    strcat(dests, exit);
            strcat(dests, " ");
	    // To destination
	    strcat(pairs, exit);
            strcat(pairs, " ");
            strcat(pairs, as_dest);
	    strcat(pairs, " ");
	    // And back
	    strcat(pairs, as_dest);
	    strcat(pairs, " ");
        ...
        log_debug(LD_CIRC, "[MONO]: Entries: %d.", smartlist_len(as_entry));

	// Paths from src to entries (and vise versa)
        for (i = 0; i < smartlist_len(as_entry); ++i) {
            //log_debug(LD_CIRC, "[MONO]: %d| %s.", i, (char*)smartlist_get(as_entry, i));
            char *entry = (char*)smartlist_get(as_entry, i);
	    strcat(dests, entry);
            strcat(dests, " ");
            // To source
            strcat(pairs, entry);
	...
	log_debug(LD_CIRC, "[MONO]: Dests| %s.", dests);
	log_debug(LD_CIRC, "[MONO]: Pairs| %s.", pairs);

	struct timeval start, stop;
	gettimeofday(&start, NULL);	
	int qlen = smartlist_len(as_entry) * 2 + smartlist_len(as_exit) * 2;
	asnset result[qlen];
        get_aspath(dests, pairs, qlen, result);
        int j;
	gettimeofday(&stop, NULL);

	float bgp_sim_time = (float)(((stop.tv_sec-start.tv_sec)*1000000 + (stop.tv_usec))-start.tv_usec);

	FILE *bgp_time = fopen(bgp_time_filename, "a");
	fprintf(bgp_time, "%s \t %f \n", dest_addr, bgp_sim_time);
	fclose(bgp_time);

        // To have exit and entry parts
        relyasns exit_parts[smartlist_len(as_exit)];
        int q = 0;
        for (i = 0; i < smartlist_len(as_exit) * 2; i+=2) {
            exit_parts[q].to = &result[i];
            exit_parts[q].back = &result[i+1];
            //log_debug(LD_CIRC, "[MONO]: LEN0 %s | %s", exit_parts[q].to->src, exit_parts[q].to->dst);
            //log_debug(LD_CIRC, "[MONO]: LEN1 %ld", exit_parts[q].to->len, exit_parts[q].back->len);
            exit_parts[q].len = exit_parts[q].to->len + exit_parts[q].back->len;
            //log_debug(LD_CIRC, "[MONO]: LEN2 %ld", exit_parts[q].len);
            int cnt = 0;
            for (j = 0; j < exit_parts[q].to->len; ++j) {
                exit_parts[q].all[cnt] = exit_parts[q].to->ases[j];
                cnt++;
            }
            for (j = 0; j < exit_parts[q].back->len; ++j) {
                exit_parts[q].all[cnt] = exit_parts[q].back->ases[j];
                cnt++;
            }
            q++;
        }
        q = 0;
        relyasns entry_parts[smartlist_len(as_entry)];
        for (; i < qlen; i+=2) {
            entry_parts[q].to = &result[i];
            entry_parts[q].back = &result[i+1];
            entry_parts[q].len = entry_parts[q].to->len + entry_parts[q].back->len;
            int cnt = 0;
            for (j = 0; j < entry_parts[q].to->len; ++j) {
                entry_parts[q].all[cnt] = entry_parts[q].to->ases[j];
                cnt++;
            }
            for (j = 0; j < entry_parts[q].back->len; ++j) {
                entry_parts[q].all[cnt] = entry_parts[q].back->ases[j];
                cnt++;
            }
            q++;
        }
	...
	// Small check
        log_debug(LD_CIRC, "[MONO]: SC from %s to %s: %ld ...", result[0].src, result[0].dst, result[0].ases[0]);
        log_debug(LD_CIRC, "[MONO]: SC from %s to %s: %ld ...", entry_parts[0].to->src, entry_parts[0].to->dst, entry_parts[0].to->ases[0]);
        // Now we can test each combination on having an attacker
        int paths = smartlist_len(as_entry) * smartlist_len(as_exit); // number of entry-exit combinations
        double** S; // matrix attackers x paths for LP
        S = malloc(paths * sizeof(double*));

        /*
        for (i = 0; i < paths; ++i) {
            S[i] = malloc(paths * sizeof(double));
            for (j = 0; j < paths; ++j) {
                S[i][j] = 0;
            }
        }
        */
        ...
        smartlist_t *attackers = smartlist_new();
        int x, y;
        int cntAttacked = 0;
        int cntFree = 0;
        int pnum = 0; // path-combination index
        int pathDetails[paths][2];
	int attacker_free_paths[paths][2];
        // iterating entry options
        for (x = 0; x < smartlist_len(as_entry); x++) 
	{
            // iterating exit options
            for (y = 0; y < smartlist_len(as_exit); y++) 
	    {
                pathDetails[pnum][0] = x;
                pathDetails[pnum][1] = y;
		//log_debug(LD_CIRC,"[TIP] Checking attackers. Path number: %d . Pair : %d %d" , pnum, pathDetails[pnum][0], pathDetails[pnum][1]);
		attacker_free_paths[pnum][0]=-1;
		attacker_free_paths[pnum][1]=-1;
		int hasAttacker = 0;
                // check for attacker
                for (j = 0; j < entry_parts[x].len; j++) 
		{
                    for (i = 0; i < exit_parts[y].len; i++) 
		    {
			//log_debug(LD_CIRC, "[TIP] Entry AS: %d , Exit AS: %d ", entry_parts[x].all[j], exit_parts[y].all[j]);
                        // detected to de present on both segments
                        if (entry_parts[x].all[j] == exit_parts[y].all[i]) 
			{
                            hasAttacker = 1;
                            int a;
                            for (a = 0; a < smartlist_len(attackers); a++) 	
			    {
                                long tmp = *(long *)smartlist_get(attackers, a);
                                if (tmp == entry_parts[x].all[j]) 
				{
                                    break;
                                }
                            }
                            if (a != smartlist_len(attackers)) 
			    {
                                // found an existent attacker
                                S[a][pnum] = 1;
                            }
                            else 
			    {
                                // adding new attacker and row
                                smartlist_add(attackers, &entry_parts[x].all[j]);
                                S[a] = malloc(paths * sizeof(double));
                                for (j = 0; j < paths; j++) 
				{
                                    S[a][j] = 0;
                                }
		...
		{
                    cntFree++;
		    attacker_free_paths[pnum-1][0]=x;
		    attacker_free_paths[pnum-1][1]=y;
                }
            }
        }

        log_debug(LD_CIRC, "[LP] Statistics: %ld (free) / %ld (attacked)", cntFree, cntAttacked);

        int m = smartlist_len(attackers);
        int n = paths;

	fprintf(attacks, " Attacker Free Paths: %ld , Attacked Paths: %ld , Number of attackers: %ld , Destination AS : %s\n\n", cntFree, cntAttacked, m, as_dest);
	fclose(attacks);

        double res[10000];
        /* Solving LP only if there are no attacker free paths. Otherwise use one of the attacker free paths at random*/
	if(cntFree==0)
        	solve_lp(S, n, m, res);
	else
	{
		/*For each attacker free path, set res[i] = 1/cntFree. Otherwise res[i] = 0*/
		for(int i = 0 ; i < paths ; i ++)
		{
			if((attacker_free_paths[i][0] == -1) && (attacker_free_paths[i][1] == -1))
			{	
				res[i] = 0;
				log_debug(LD_CIRC,"[TIP] Path %d is attacked. RES[%d] = %f. Attacker flags: %d %d", i, i, res[i], attacker_free_paths[i][0], attacker_free_paths[i][1]);
			}
			else if ((attacker_free_paths[i][0] >= 0) && (attacker_free_paths[i][1] >= 0))
			{
				res[i] = (double)(1/(double)cntFree);
				log_debug(LD_CIRC,"[TIP] Path %d is not attacked. RES[%d] = %f. Attacker flags: %d %d", i, i, res[i], attacker_free_paths[i][0], attacker_free_paths[i][1]);
			}
			else
			{
				log_debug(LD_CIRC, "[TIP] This is weird and should not happen. Path# %d, Attacker flags: %d %d", i, attacker_free_paths[i][0], attacker_free_paths[i][1]);
			}
			log_debug(LD_CIRC,"[TIP] There was no need to use the LP. RES[%d] = %f", i, res[i]);
		}
	}

	/* Blame Rishab if this breaks everything. Buy him a drink if it works.*/
	int entry_index, exit_index, invalid_entryAS = 0, invalid_exitAS = 0;
        int no_entry = 0 , no_exits = 0;
        no_entry = smartlist_len(entry_nodes);
        no_exits = smartlist_len(good_exits);
	
	/*Get the results of the LP here*/
	int number_of_pairs = no_entry*no_exits;
	double lp_bw_mins[number_of_pairs];
	double lp_sec_prob[number_of_pairs];

       	for(int i  = 0 ; i < number_of_pairs ; i ++)
	{
		int curr_entry = pathDetails[i][0];
		int curr_exit = pathDetails[i][1];
		lp_sec_prob[i] = res[i];
		//This picks the min of the entry and exit node bw. Probably not so smart.
		//lp_bw_mins[i] = MIN(entry_node_bw[curr_entry], exit_node_bw[curr_exit]); 
		//This picks the weighted val of entry and exit node bw. Much smarter.
		lp_bw_mins[i] = entry_node_bw[curr_entry] * exit_node_bw[curr_exit];
		if((attacker_free_paths[i][0]>=0)&&(attacker_free_paths[i][1]>=0)) //If this is an attacker free path, then just use the lp_bw_min_score
			lp_sec_prob[i] = lp_bw_mins[i];
		log_debug(LD_CIRC, "[TIP] Pair: %d, Entry index: %d, Exit index: %d, LP Probability: %f, BW Min: %f", i, curr_entry, curr_exit, lp_sec_prob[i], lp_bw_mins[i]);
	}

	/*Get the probability distributions for the BW mins and the LP here*/
	double bw_dist[number_of_pairs], lp_dist[number_of_pairs];
	generate_distribution(lp_bw_mins, number_of_pairs, 0, bw_dist);
	generate_distribution(lp_sec_prob, number_of_pairs, 0, lp_dist);

	/*Tune the final distribution based on alpha value here*/
	double tuned_dist[number_of_pairs];
	tune_distributions(bw_dist, lp_dist, number_of_pairs, alpha, tuned_dist);
	/*Select the entry-exit pair from the tuned distributions and set them here.*/
        do
        {
	    int pair_index = pick_from_dist(tuned_dist, number_of_pairs);
	    entry_index = pathDetails[pair_index][0];
	    exit_index = pathDetails[pair_index][1];
            log_debug(LD_CIRC,"[TIP - BUG] Current node selections (en/ex): %d/%d, Alpha Probability: %f, BW Prob: %f, LP Prob: %f, alpha: %f , Pair Index: %d, Attacker Flags: %d %d", entry_index, exit_index, tuned_dist[pair_index], bw_dist[pair_index], lp_sec_prob[pair_index], alpha, pair_index, attacker_free_paths[pair_index][0], attacker_free_paths[pair_index][1]);
	   for(int i = 0 ; i < entry_parts[entry_index].len ; i ++)
	    {
		log_debug(LD_CIRC, "[TIP - BUG] ASes on path to entry: %d are: %ld", entry_index, entry_parts[entry_index].all);	
	    } 
	    for(int i = 0 ; i < exit_parts[exit_index].len ; i ++)
	    {
		log_debug(LD_CIRC, "[TIP - BUG] ASes on path to entry: %d are: %ld", exit_index, exit_parts[entry_index].all);	
	    } 
	    
	    for(int i= 0 ; i < smartlist_len(attackers) ; i ++)
	    {
		if (S[i][pair_index] == 1)
			log_debug(LD_CIRC, "[TIP - BUG] Attackers on pair index: %d are :%ld", pair_index, *(long *)smartlist_get(attackers, i));
	    }
            invalid_entryAS = strcmp(smartlist_get(as_entry, entry_index), "-1");
            invalid_exitAS = strcmp(smartlist_get(as_exit, exit_index), "-1");
        }while((invalid_entryAS ==0) || (invalid_exitAS == 0));

	char entry_ip[16], exit_ip[16];
        strcpy(entry_ip, parse_ip(node_describe(smartlist_get(entry_nodes, entry_index))));
        strcpy(exit_ip, parse_ip(node_describe(smartlist_get(good_exits, exit_index))));

        fprintf(bw_file, "\nDestination AS: %s , Entry IP: %s , Entry AS: %s , Entry BW: %f , Exit IP: %s , Exit AS: %s , Exit BW: %f", as_dest, entry_ip, do_whois(entry_ip),  entry_node_bw[entry_index], exit_ip, do_whois(exit_ip), exit_node_bw[exit_index]);
	fclose(bw_file);

        exit = extend_info_from_node(smartlist_get(good_exits, exit_index),0);
        entry = extend_info_from_node(smartlist_get(entry_nodes, entry_index), 0);
        //log_debug(LD_CIRC, "[TIP] Source (AS%s/IP%s) -> entry (%s), exit(%s) -> Dest(AS%s/IP%s)", as_src, ip_src, entry->nickname, exit->nickname, as_dest, ip_dest);
        ...
        */
        ...
        log_debug(LD_CIRC, "[TIP] Entry node list created. Selecting %s/%s as the entry/exit nodes.", entry->nickname, exit->nickname);
    }
    if(as_onion_populate_cpath(circ, entry) < 0)
    {
        circuit_mark_for_close(TO_CIRCUIT(circ), END_CIRC_REASON_NOPATH);
        return NULL;
    }
  ...
  {
      circ = origin_circuit_init(purpose, flags);
      if (onion_pick_cpath_exit(circ, exit) < 0 || onion_populate_cpath(circ) < 0)
      {
        circuit_mark_for_close(TO_CIRCUIT(circ), END_CIRC_REASON_NOPATH);
        return NULL;
      }
...
//{
//  origin_circuit_t *circ;
//  extend_info_t *set_exit;
//  node_t *exit_node;
//  int err_reason = 0, number_exits = 0;
//  log_debug(LD_CIRC, "[TIP] Establishing new circuit for purpose: %d", purpose);
//  log_debug(LD_CIRC, "[TIP] Establishing new circuit with exit: %s", extend_info_describe(exit));
//
//  if(0)//flags & TOR_AS_PATHS)
//  {
//    circ = origin_circuit_init(purpose, flags);
//    cpath_build_state_t *state = circ->build_state;
//
//    //log_debug(LD_CIRC, "[TIP] AS_PATHS flag is set. Ignoring request to use exit server: %s", extend_info_describe(exit));
//    log_debug(LD_CIRC, "[TIP] Generating a list of usable exit nodes.");
//
//    smartlist_t *good_exits = smartlist_new();
//    good_exits = return_all_good_exit_server_general(state->need_uptime, state->need_capacity);
//    number_exits = smartlist_len(good_exits);
//    log_debug(LD_CIRC, "[TIP] Number of available exit nodes: %d", number_exits);
//    SMARTLIST_FOREACH_BEGIN(good_exits, node_t *, node){
//      //log_debug(LD_CIRC, "[TIP] Exit %s is available", node_describe(node));
//    }SMARTLIST_FOREACH_END(node);
//
//    int cpath_status = 1;
//    if(strcmp(extend_info_describe(exit),"<null>")!=0)
//    {
//        log_debug(LD_CIRC, "[TIP] Exit not NULL. Using given exit: %s", extend_info_describe(exit));
//        cpath_status = onion_pick_cpath_exit(circ, exit);
//    }
//    else
//    {
//        exit_node = smartlist_get(good_exits, 1);
//        log_debug(LD_CIRC, "[TIP] Exit is NULL. Temporarily setting exit to: %s", node_describe(exit_node));
//        set_exit = extend_info_from_node(exit_node, 0);
//        cpath_status = onion_pick_cpath_exit(circ, set_exit);
//    }
//    if(cpath_status < 0)
//    {
//       log_debug(LD_CIRC, "[TIP] Error while setting cpath exit (AS Tor)");
//       circuit_mark_for_close(TO_CIRCUIT(circ), END_CIRC_REASON_NOPATH);
//       return NULL;
//    }
//
...
//     const int using_ntor = circuits_can_use_ntor();
//     #define MAX_POPULATE_ATTEMPTS 32
//     while(cpath_status == 0)
//     {
//        //Keep building the circuit while it is incomplete and there are no errors.
//        state = circ->build_state;
//        uint8_t purpose = circ->base_.purpose;
//        int cur_len = circuit_get_cpath_len(circ);
//        extend_info_t *info = NULL;
//        log_debug(LD_CIRC, "[TIP] Circuit ID: %d", circ->global_identifier);
//        if (cur_len >= state->desired_path_len)
//        {
//            log_debug(LD_CIRC, "[TIP] Path is complete: %d steps long", state->desired_path_len);
//            cpath_status = 1;
//            break;
//        }
...
//        {
//            /* Picking last node */
//            log_debug(LD_CIRC, "[TIP] Path is %d long; we want %d", cur_len, state->desired_path_len);
...
//            /* picking first node */
//            const or_options_t *options = get_options();
//            log_debug(LD_CIRC, "[TIP] Picking entry node.");
//            const smartlist_t *entry_nodes = smartlist_new();
//            int number_entry = 0;
//            //Check if we have to use entry guards.
//            if (state && options->UseEntryGuards && (purpose != CIRCUIT_PURPOSE_TESTING || options->BridgeRelay))
...
//            }
//
//            log_debug(LD_CIRC, "[TIP] Total entry nodes: %d", smartlist_len(entry_nodes));
//            if(smartlist_len(entry_nodes) < 1)
//                log_debug(LD_CIRC, "[TIP] Unable to pick an entry node.");
//            const node_t *entry = smartlist_get(entry_nodes, smartlist_len(entry_nodes) - 1);
//            log_debug(LD_CIRC, "[TIP] Entry node list created. Selecting %s as the entry node.", node_describe(entry));
//            info = extend_info_from_node(entry, server_mode(get_options()) == 0);
//            tor_assert(info);
//        }
//        else
//        {
...
//        if (!info)
//        {
//            log_warn(LD_CIRC,"Failed to find node for hop %d of our path. Discarding this circuit.", cur_len);
//            cpath_status = -1;
//        }
//        log_debug(LD_CIRC,"[TIP] Chose router %s for hop %d (exit is %s)", extend_info_describe(info), cur_len+1, build_state_get_exit_nickname(state));
...
//        if (circ->build_state->desired_path_len <= 1 || ! using_ntor)
//        {
//            log_debug(LD_CIRC, "This circuit doesn't need/shouldn't be forced to have an ntor hop");
//        }
//        /* This circuit has an ntor hop. great! */
//        else if (circuit_cpath_supports_ntor(circ))
//        {
//            log_debug(LD_CIRC, "This circuit has an ntor hop");
//        }
//        /* No node in the circuit supports ntor.  Have we already tried too many
//        * times? */
//        else if (++n_tries >= MAX_POPULATE_ATTEMPTS)
//        {
//            cpath_status = -1;
//            log_debug(LD_CIRC, "[TIP] Unable to build a circuit after many attempts");
//        }
//        /* Clear the path and retry */
//        else
//        {
//            circuit_clear_cpath(circ);
//            log_debug(LD_CIRC, "[TIP] Unable to build a circuit. Retrying.");
//            goto build_circuit_start;
//        }
//     }
//     if(cpath_status == -1)
//     {
//        log_debug(LD_CIRC, "[TIP] Circuit building error.");
//        circuit_mark_for_close(TO_CIRCUIT(circ), END_CIRC_REASON_NOPATH);
//        return NULL;
//     }
//
...
//
//      if ((err_reason = circuit_handle_first_hop(circ)) < 0) {
//        log_debug(LD_CIRC, "[TIP] Error while handling first hop");
//        circuit_mark_for_close(TO_CIRCUIT(circ), -err_reason);
//        return NULL;
//      }
//      return circ;
...
circuit_handle_first_hop(origin_circuit_t *circ)
{
  ...
  log_debug(LD_CIRC,"[TIP] Looking for firsthop '%s'",
  ...
  if (!n_chan) {
    ...
    log_info(LD_CIRC, "[TIP] Next router is %s: %s",
    ...
    if (should_launch) {
      ...
      if (!n_chan) { /* connect failed, forget the whole thing */
        log_info(LD_CIRC,"[TIP] connect to firsthop failed. Closing.");
        return -END_CIRC_REASON_CONNECTFAILED;
      }
    }

    log_debug(LD_CIRC,"[TIP] connecting in progress (or finished). Good.");
    ...
    log_debug(LD_CIRC,"[TIP] Conn open. Delivering first onion skin.");
    if ((err_reason = circuit_send_next_onion_skin(circ)) < 0) {
      log_info(LD_CIRC,"[TIP] circuit_send_next_onion_skin failed.");
                                    ...
                                    int *need_capacity)
{
  ...
  *need_capacity = 1;
  ...
  smartlist_free(sl);
  //Always returning 1, so predictive circuit building is disabled.
  //This is only to disable building of exit circuits.
  return 1;
  //return enough;
}
...
//             router->nickname);
  ...
  log_info(LD_CIRC,
           "[TIP] Found %d servers that might support %d/%d pending connections.",
...
//                 try, router->nickname);
  ...
  tor_free(n_supported);
  if (node) {
    log_info(LD_CIRC, "[TIP] Chose exit server '%s'", node_describe(node));
    return node;
  }
  if (options->ExitNodes) {
    log_warn(LD_CIRC,
...
onion_pick_cpath_exit(origin_circuit_t *circ, extend_info_t *exit)
{
  ...
  if (state->onehop_tunnel) {
    log_debug(LD_CIRC, "[TIP] Launching a one-hop circuit for dir tunnel.");
  ...
  if (exit) { /* the circuit-builder pre-requested one */
    ...
    log_info(LD_CIRC,"[TIP] Using requested exit node '%s'",
  ...
  log_debug(LD_CIRC, "[TIP] Chosen exit node: %s", extend_info_describe(exit));
...
choose_good_entry_server(uint8_t purpose, cpath_build_state_t *state)
{
  ...
  router_crn_flags_t flags = CRN_NEED_GUARD|CRN_NEED_DESC;
  const node_t *node;
  log_debug(LD_CIRC, "[TIP] @choose_good_entry_server()");
  log_debug(LD_CIRC, "[TIP] UseEntryGuards: %d",options->UseEntryGuards);
  ...
  if (state && options->UseEntryGuards &&
    ...
    /* This request is for an entry server to use for a regular circuit,
     * and we use entry guard nodes.  Just return one of the guard nodes.  */
     log_debug(LD_CIRC, "[TIP] This request is for an entry server to use for a regular circuit. Returning one of the entry guards.");
     choice = choose_random_entry(state);
     log_debug(LD_CIRC, "[TIP] Chosen entry node: %s", node_describe(choice));
     return choice;
  }
...
onion_extend_cpath(origin_circuit_t *circ)
{
  ...
  log_debug(LD_CIRC, "[TIP] Circuit ID: %d", circ->global_identifier);
  if (cur_len >= state->desired_path_len) {
    log_debug(LD_CIRC, "[TIP] Path is complete: %d steps long",
              state->desired_path_len);
    return 1;
  }

  log_debug(LD_CIRC, "[TIP] Path is %d long; we want %d", cur_len,
            state->desired_path_len);
  ...
  if (cur_len == state->desired_path_len - 1) { /* Picking last node */
    ...
    log_debug(LD_CIRC, "[TIP] Chosen entry node: %s", node_describe(r));
    if (r) {
...
static int
as_onion_extend_cpath(origin_circuit_t *circ, extend_info_t *entry)
{
  uint8_t purpose = circ->base_.purpose;
  cpath_build_state_t *state = circ->build_state;
  int cur_len = circuit_get_cpath_len(circ);
  ...
  { /* picking first node */
    if(strcmp(extend_info_describe(entry),"<null>")==0)
    {
        const node_t *r =  choose_good_entry_server(purpose, state);
        info = extend_info_from_node(r, server_mode(get_options()) == 0);
        log_debug(LD_CIRC, "[TIP] Standard Chosen entry node: %s", extend_info_describe(info));
        tor_assert(info);
    }
    else
    {
        ...
        tor_assert(info);
        log_debug(LD_CIRC, "[TIP] AS Chosen entry node: %s", extend_info_describe(info));
    }
  }
  else
  {
  ...
  if (!info) {
    log_warn(LD_CIRC,"[TIP] Failed to find node for hop %d of our path. Discarding "
             "this circuit.", cur_len);
    return -1;
  }

  log_debug(LD_CIRC,"[TIP] Chose router %s for hop %d (exit is %s)",
            extend_info_describe(info),
            cur_len+1, build_state_get_exit_nickname(state));
