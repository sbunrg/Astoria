...
double *smartlist_return_bw_weights(const smartlist_t *sl, bandwidth_weight_rule_t rule)
{
    u64_dbl_t *bandwidths=NULL;
    compute_weighted_bandwidths(sl, rule, &bandwidths);
    return bandwidths;
}
...
/** Return a list of running nodes from the nodelist. Never
 * pick nodes that are in
 * <b>excludedsmartlist</b>, or which matches <b>excludedset</b>,
 * even if they are the only nodes available.
 * If <b>CRN_NEED_UPTIME</b> is set in flags and any router has more than
 * a minimum uptime, return one of those.
 ...
 */
smartlist_t *
return_all_but_excluded(smartlist_t *excludedsmartlist,
                          routerset_t *excludedset,
                          router_crn_flags_t flags)
{ /* XXXX MOVE */
  const int need_uptime = (flags & CRN_NEED_UPTIME) != 0;
  ...
  smartlist_subtract(sl,excludednodes);
  log_debug(LD_CIRC, "We removed %d excludednodes, leaving %d nodes.", smartlist_len(excludednodes), smartlist_len(sl));

  if (excludedsmartlist) {
    smartlist_subtract(sl,excludedsmartlist);
    log_debug(LD_CIRC, "We removed %d excludedsmartlist, leaving %d nodes.", smartlist_len(excludedsmartlist), smartlist_len(sl));
  }
  if (excludedset) {
    routerset_subtract_nodes(sl,excludedset);
  ...
  if (smartlist_len(sl) == 0 && (need_uptime || need_capacity || need_guard)) {
    /* try once more -- recurse but with fewer restrictions. */
    log_info(LD_CIRC,
             "We couldn't find any live%s%s%s routers; falling back "
             "to list of all routers.",
             ...
             need_guard?", guard":"");
    flags &= ~ (CRN_NEED_UPTIME|CRN_NEED_CAPACITY|CRN_NEED_GUARD);
    sl = return_all_but_excluded(excludedsmartlist, excludedset, flags);
  }
  smartlist_free(excludednodes);
  return sl;
}
