...
int compare_ip_addr(char *ip1, char *ip2, int levels)
{
    int result = -1, count = 0;
    char *temp1 = strdup(ip1), *temp2 = strdup(ip2);
    char **ip_t1 = &temp1, **ip_t2 = &temp2;
    char *tok1, *tok2;
    //Split them into tokens and compare levels till there is a difference
    while (tok1 = strsep(ip_t1, "."))
    {
        tok2 = strsep(ip_t2, ".");
...
/**
  * Take two IP addresses and return 1 if they share the same AS, and 0 otherwise.
**/
//char ip_as_map[255][255][255][255][10];

char *parse_as(char *result)
{
    //log_debug(LD_CIRC, "[TIP] Result of whois: %s", result);
    char *copy = strdup(result), *token;
    //int token_count = 0;
    char **copy_tokens = &copy;
    token = strsep(copy_tokens, "|");
    //log_debug(LD_CIRC, "[TIP] AS extracted: %s", token);
    return token;
}
...
char *parse_batch_result_as(char *line)
{
    char *copy = strdup(line), *token;
    log_debug(LD_CIRC, "[TIP] Input: %s", line);

    char **copy_tokens = &copy;
    token = strsep(copy_tokens, "|");
    log_debug(LD_CIRC, "[TIP] Extracted AS: %s", token);
    return token;
}
char *parse_batch_result_ip(char *line)
{
    char *copy = strdup(line), *token;
    log_debug(LD_CIRC, "[TIP] Input: %s", line);
    char **copy_tokens = &copy;
    token = strsep(copy_tokens, "|");
    token = strsep(copy_tokens, "|");
    log_debug(LD_CIRC, "[TIP] Extracted IP: %s", token);
...
{
    char query[100], strip_command[100], savecol_command[100];
    strcpy(query, "netcat whois.cymru.com 43 < ");
    strcat(query, input);
    strcat(query, "|sort -n >");
    strcat(query, output);
    log_debug(LD_CIRC, "[TIP] Making netcat batch query: %s", query);
    log_debug(LD_CIRC, "[TIP] Query result: %d",system(query));
    strcpy(savecol_command, "cut -d '|' -f 1,2 ");
    strcat(savecol_command, output);
    strcat(savecol_command,"| sed 's/|//' > temp");
    log_debug(LD_CIRC, "[TIP] sed result: %d",system(savecol_command));

    FILE *f_output = fopen("temp", "r");
    //log_debug(LD_CIRC, "[TIP] Extract command result: %d",system(savecol_command));
    //log_debug(LD_CIRC, "[TIP] Extract result: %d",system(savecol_command));

    char line_result[80], as[10], ip[16];
    int count = 0;
    fscanf(f_output, "%[^\n]", line_result);
    //log_debug(LD_CIRC, "[TIP] First line: %s", line_result);

    while(fscanf(f_output, "%s %s", as, ip)!= EOF)
    {
        install(ip, as);
        //log_debug(LD_CIRC,"[TIP] File: %s, ip: %s , as: %s", output, ip, as);
    }
    fclose(f_output);
}
int split_ipaddr(char *input_ip, int index)
{
    int ip_int[4], count = 0, wordcount = 0;
    char buf[3];
    for(int i = 0 ; i < strlen(input_ip) ; i ++)
    {
        if(input_ip[i]!='.')
        {
            buf[wordcount] = input_ip[i]; buf[wordcount+1]='\0';
            wordcount++;
            //log_debug(LD_CIRC,"[TIP] adding %c to %s", ip[i], buf);
        }
        else if(input_ip[i] == '.' || i == strlen(input_ip)-1 )
        {
            ip_int[count] = atoi(buf);
            //log_debug(LD_CIRC,"[TIP] ip[%d]:%d (%s)", count, ip_int[count], buf);
            count++; wordcount = 0;
            strcpy(buf,"");
        }
        ip_int[3] = atoi(buf);
    }
    //log_debug(LD_CIRC,"[TIP] IP %s as ints: %d %d %d %d", input_ip, ip_int[0], ip_int[1], ip_int[2], ip_int[3]);
    return ip_int[index];
}
char *ip_tostring(int *ip, int suffix)
{
    char ipstring[25];
    sprintf(ipstring, "%d.%d.%d.%d/%d", ip[0],ip[1],ip[2],ip[3],suffix);
//    sprintf(str, "%d", ip[0]);
//    strcat(ipstring,str);
//    log_debug(LD_CIRC,"[TIP] STR: %s, IPSTRING: %s, INT: %d", str, ipstring, *ip);
//    strcat(ipstring,".");
//    sprintf(str, "%d", ip[1]);
//    strcat(ipstring,str);
//    log_debug(LD_CIRC,"[TIP] STR: %s, IPSTRING: %s, INT: %d", str, ipstring, ip[1]);
//    strcat(ipstring,".");
//    sprintf(str, "%d", ip[2]);
//    strcat(ipstring,str);
//    log_debug(LD_CIRC,"[TIP] STR: %s, IPSTRING: %s, INT: %d", str, ipstring, ip[2]);
//    strcat(ipstring,".");
//    sprintf(str, "%d", ip[3]);
//    strcat(ipstring,str);
//    log_debug(LD_CIRC,"[TIP] STR: %s, IPSTRING: %s, INT: %d", str, ipstring, ip[3]);
//    if(suffix > 0)
//    {
//        strcat(ipstring,"/");
//        sprintf(str, "%d", 32-suffix);
//        strcat(ipstring,str);
//    }
    //log_debug(LD_CIRC,"[TIP] IP %d %d %d %d as string %s", ip[0], ip[1], ip[2], ip[3], ipstring);
    return ipstring;
}
int max(int a, int b)
{
...
char * int2bin(int i)
{
    size_t bits = 8;//sizeof(int) * CHAR_BIT;
    ...
    for(; bits--; u >>= 1)
    	str[bits] = u & 1 ? '1' : '0';
    //log_debug(LD_CIRC, "[TIP] Binary rep of %d is %s", i,str);
...
char *formcidrstring(char *binary, int suffix)
{
    int count = 0;
    char copy_bin[35];
    strncpy(copy_bin, binary,35);
    //log_debug(LD_CIRC, "[TIP] Binary: %s, Copied: %s", binary, copy_bin);
    for(int i = 34 ; i >= 0 ; i --)
    {
        //log_debug(LD_CIRC, "[TIP] Index: %d, suffix: %d, count: %d, Char: %c", i, suffix, count, copy_bin[i]);
        if(count < suffix)
        {
            if(copy_bin[i]!='.')
            {
    ...
    //log_debug(LD_CIRC, "[TIP] New IP (Binary) CIDR: %s/%d",copy_bin, 32-suffix);
    char temp[8], integer_ip[20];
    int temp_[4]={0,0,0,0};
    for(int i = 0 ; i < 4 ; i ++)
    {
        strncpy(temp, copy_bin+(i+i*8), 8);
        //log_debug(LD_CIRC, "[TIP] %d th 8 bits: %s", i+1, temp);
        temp_[i]= bin2int(temp);
        //log_debug(LD_CIRC, "[TIP] %d th 8 bits as int: %d", i+1, temp_[i]);
    }
    char *cidr = ip_tostring(temp_, 32-suffix);
    //log_debug(LD_CIRC, "[TIP] New IP CIDR: %s",cidr);
...
{
    //Check a simple query to see if we loaded the AS-IP map DB in memory.
    nlist *test = lookup("24.75.21.0/24");
    if(test == NULL)
    {
        //we have not loaded this database, need to do it now.
        log_debug(LD_CIRC, "[TIP] Loading AS-IP map into memory");
        FILE *database;
        char ip[30], as[30];
        //Update the location of this map to wherever it will be on the deployment system
        database = fopen("0315-ip-as-map.txt", "r");
        if(!database)
        {
            log_debug(LD_CIRC, "[TIP] Unable to open AS-IP mapping database. Quitting.");
            return("1");
        }
        while (fscanf(database, "%s %s", as, ip) != EOF)
        {
            install(ip, as);
        }
        fclose(database);
        log_debug(LD_CIRC, "[TIP] Done loading AS-IP map into memory");
    }
    //At this point, the mapping database is in memory for sure.
    //Now, we want to find the longest prefix match in the database.
    nlist *direct_map = lookup(ip1);
    if(direct_map != NULL)
    {
        //log_debug(LD_CIRC, "[TIP] Found a direct IP-AS map.");
        return direct_map->defn;
    }
    //else
        //log_debug(LD_CIRC, "[TIP] Going to check different suffixes. No direct map found");
    int ip[4]={0,0,0,0}, temp;
    char ip_binary[36];
    strcpy(ip_binary, "");
    for(int i = 0 ; i < 4 ; i ++)
    {
        ip[i] = split_ipaddr(ip1, i);
        temp = ip[i];
        char *result = int2bin(temp);
        int temp = bin2int(result);
        //log_debug(LD_CIRC, "[TIP] Binary rep of %d is %s (%d)", ip[i], result, temp);
        strcat(ip_binary, result);
        if(i!=3)
            strcat(ip_binary, ".");
    }
    //log_debug(LD_CIRC, "[TIP] Binary rep of %s is %s ",ip1, ip_binary);

    int suffix = 1;
    while(suffix <= 28)
    {
        //char *query = formcidrstring(ip_binary, suffix);
        //log_debug(LD_CIRC, "[TIP] Query: %s", query);
        //log_debug(LD_CIRC, "[TIP] Query: %s", formcidrstring(ip_binary, suffix));
        nlist *result_nlist = lookup(formcidrstring(ip_binary, suffix));
        if(result_nlist == NULL)
            suffix++;
        else
        {
            //log_debug(LD_CIRC, "[TIP] Found a match at suffix: %d, AS:%s. Caching direct IP-AS map", suffix, result_nlist->defn);
            install(ip1,result_nlist->defn);
            return result_nlist->defn;
        }
    }
    log_debug(LD_CIRC,"[TIP] No entry in AS-IP map DB found. Returning AS -1.");
    install(ip1, "-1");
    return "-1";
    //log_debug(LD_CIRC, "[TIP] Getting the AS for IP %s", ip1);
    //log_debug(LD_CIRC, "[TIP] Whois query: %s", query);
    //Check if we have the AS for (truncated) IP1 in our DB, already
    //If we do, return that. Otherwise, do a new query
//    nlist *result_nlist = lookup(ip1);
//    if(result_nlist==NULL)
//    {
//        char query[100], result[200], c;
//        strcpy(query, "whois -h whois.cymru.com \"");
//        strncat(query, ip1, strlen(ip1));
//        strncat(query, "\"", 1);
//        FILE *whois;
//        whois = popen(query, "r");
//        int i = 0, newline = 0;
//        while((c = fgetc(whois)) != EOF)
//        {
//            if(c == '\n')
//                newline = 1;
//            if(newline && c != '\n')
...
//        if (pclose(whois) == -1)
//        {
//            log_debug(LD_CIRC, "[TIP] Whois lookup failed.");
//            return 0;
//        }
//        char as[10];
//        strcpy(as, parse_as(result));
//        install(ip1, as);
//        log_debug(LD_CIRC, "[TIP] We've never encountered this IP (%s) before. AS found to be: %s", ip1, as);
//        return as;
//    }
//    else
//    {
//        //log_debug(LD_CIRC, "[TIP] We've encountered this IP (%s) before. AS: %s", result_nlist->name, result_nlist->defn);
//        return result_nlist->defn;
//    }
}
int compare_as(char *ip1, char *ip2)
{
    //log_debug(LD_CIRC, "[TIP] Comparing the ASes of IPs %s and %s", ip1, ip2);
    if(strcmp(ip1, ip2) == 0)
        return 1;
    if(!ip1 || !ip2)
        return -1;
    if(strcmp(do_whois(ip1), do_whois(ip2)) == 0)
        return 1;
    else return 0;
}
...
int solve_lp(double* S[], int n, int m, double res[]) {
    ...
    lprec *lp;
    // Creating a new problem with n+1 varibales 
    if ((lp = make_lp(0,n+1)) == NULL) return 1;
    set_outputfile(lp, "lp-log");
    set_verbose(lp, NEUTRAL);
    set_add_rowmode(lp, TRUE);
    
    log_debug(LD_CIRC, "[LP] Solving: %ld x %ld", m, n);
    ...
    // Lower-bound are by default zeros
    //set_lowbo(lp,2,2);
...
int get_aspath(const char *dst, const char* pairs, int pairs_num, asnset result[pairs_num])
{
    ...
    sprintf(query, "%s -q %s<EOFc>", dst, pairs);
    log_debug(LD_CIRC, "[MONO] Query: %s.", query);
    ...
    if (sockfd < 0) {
        log_debug(LD_CIRC, "[MONO] Socket creation failed.");
        return -1;
    }

    log_debug(LD_CIRC, "[MONO] Socket created.");

    server = gethostbyname("127.0.0.1");
    if (server == NULL) {
        log_debug(LD_CIRC, "[MONO] Socket hostname failed.");
        return -1;
    }
    ...
    log_debug(LD_CIRC, "[MONO] Socket bind.");

    if (connect(sockfd, &serv_addr, sizeof(serv_addr)) < 0)
    {
        log_debug(LD_CIRC, "[MONO] Socket connect failed.");
        return -1;
    }

    log_debug(LD_CIRC, "[MONO] Socket connected.");

    n = write(sockfd, query, strlen(query));

    if (n < 0)
    {
        log_debug(LD_CIRC, "[MONO] Socket write failed.");
        return -1;
    }

    log_debug(LD_CIRC, "[MONO] Socket written.");


    int buf_ind = 0;

    while (buf_ind < 1000000) {
        n = read(sockfd, buf + buf_ind, 1000000);

        if (n < 0)
        {
            log_debug(LD_CIRC, "[MONO] Socket read failed.");
            return -1;
        }
        if (n == 0) break;
        ...
        if (!strcmp("<EOFs>", buf + (buf_ind-6))) break;
    }

    log_debug(LD_CIRC, "[MONO] Socket read.");
    
    log_debug(LD_CIRC, "[MONO] Returned: (%d) %d - %.*s.", n, buf_ind, 10, buf + (buf_ind-10));

    n = buf_ind;

    char tmp[50];
    int j = 0; // over tmp
    int r = 0; // over records
    int l = 0; // length
    int i;
    for (i = 0; i < n; ++i) {
        if (buf[i] != '\n') {
            ...
            log_debug(LD_CIRC, "[MONO]: Output line: %s.", tmp);

            if (tmp[0] == 'A') {
                sscanf(tmp, "ASes from %s to %s:", result[r].src, result[r].dst);
            }
            else if (tmp[0] == 'I' || tmp[0] == 'A' || tmp[0] == 'D' || tmp[0] == '<') {
                continue;
            }
            else if (tmp[0] == '-') {
                result[r].len = l;
                ...
                result[r].ases[l] = atol(tmp);
                //strcpy(result[r].ases[l], tmp);
                l++;
                result[r].len = l;
            }
        }
    ...
    log_debug(LD_CIRC, "[MONO]: Pairs returnd: %d, out of %d.", r, pairs_num);
...
/*
void get_aspath(const char *dst, const char* pairs, int pairs_num, asnset result[pairs_num])
{
    char query[200000];
    char buf[200];

    sprintf(query, "mono %s %s %s -q %s", BGP_SIM_PATH, CYCLOPS_CAIDA, dst, pairs);
    log_debug(LD_CIRC, "[MONO] Query: %s.", query);
    ...
    // Getting the whole output
    while ((c = fgetc(mono)) != EOF)
    {
        if (c != '\n')
        {
            buf[i] = c;
	...
	else
	{
            buf[i] = '\0';
	    i = 0;
	    log_debug(LD_CIRC, "[MONO]: Output line: %s.", buf);
    ...
    if (pclose(mono) == -1)
    {
        log_debug(LD_CIRC, "[TIP] Mono failed.");
        return;
    }

    log_debug(LD_CIRC, "[MONO]: Pairs returnd: %d, out of %d.", j, pairs_num);
}
*/
...
circuit_get_best(const entry_connection_t *conn, int must_be_open, uint8_t purpose, int need_uptime, int need_internal)
{
  ...
  SMARTLIST_FOREACH_BEGIN(circuit_get_global_list(), circuit_t *, circ) {
    ...
    /* If the connection is for a different range of IPs than the circuit destination,
     * we keep looking for another circuit*/
     if(origin_circ->dest_address && (compare_as(origin_circ->dest_address, conn->socks_request->address) != 1))
        continue;
  ...
  if (!best && intro_going_on_but_too_old)
    log_info(LD_REND|LD_CIRC, "[TIP] There is an intro circuit being created "
             "right now, but it has already taken quite a while. Starting "
             "one in parallel.");
  if(best)
    log_debug(LD_CIRC, "[TIP] Circuit %d (destination: %s) is best for connection to %s", best->global_identifier, best->dest_address, conn->socks_request->address);
  else
    log_debug(LD_CIRC, "[TIP] Unable to find a suitable circuit for this connection, yet.");
  //return NULL;
  return best;
}
...
circuit_predict_and_launch_new(void)
{
  ...
  SMARTLIST_FOREACH_BEGIN(circuit_get_global_list(), circuit_t *, circ) {
    ...
    origin_circ = TO_ORIGIN_CIRCUIT(circ);
    log_debug(LD_CIRC, "[TIP] Iterating through circuit: %d. Is a GP-Circuit.", origin_circ->global_identifier);
    if (origin_circ->unusable_for_new_conns)
      continue;
    build_state = origin_circ->build_state;
    if (build_state->onehop_tunnel)
  ...
  {
    log_debug(LD_CIRC, "[TIP] We have enough circuits. Not making any more.");
    return;
  } /* we already have many, making more probably will hurt */
  ...
  if (!circuit_all_predicted_ports_handled(now, &port_needs_uptime,
    ...
    log_info(LD_CIRC, "[TIP] Have %d clean circs (%d internal), need another exit circ.", num, num_internal);
    /*
    Disable the following line for standard Tor circuit building
    */
    flags |= TOR_AS_PATHS;
  ...
  if (num_rend_services() && num_uptime_internal < 3
    ...
    log_info(LD_CIRC,
             "[TIP] Have %d clean circs (%d internal), need another internal "
             "circ for my hidden service (server).",
             num, num_internal);
    circuit_launch(CIRCUIT_PURPOSE_C_GENERAL, flags);
    return;
  }
  ...
  if (rep_hist_get_predicted_internal(now, &hidserv_needs_uptime,
    ...
    log_info(LD_CIRC,
             "[TIP] Have %d clean circs (%d uptime-internal, %d internal), need"
             " another hidden service circ. (client)",
  ...
  if (router_have_consensus_path() != CONSENSUS_PATH_UNKNOWN) {
    if (num < MAX_UNUSED_OPEN_CIRCUITS-2 &&
      ...
      if (router_have_consensus_path() == CONSENSUS_PATH_INTERNAL)
        flags |= CIRCLAUNCH_IS_INTERNAL;
      log_info(LD_CIRC,
               "[TIP] Have %d clean circs need another buildtime test circ.", num);
      circuit_launch(CIRCUIT_PURPOSE_C_GENERAL, flags);
      return;
    }
  }
...
{
  log_debug(LD_CIRC, "[TIP] Launching new circuit for purpose: %d", purpose);
  if(flags & TOR_AS_PATHS)
    log_debug(LD_CIRC, "[TIP] Launching new circuit using Tor AS Paths");
...
circuit_launch_by_extend_info(uint8_t purpose,
                              ...
                              int flags)
{
  log_debug(LD_CIRC, "[TIP] Launching new circuit with exit: %s", extend_info_describe(extend_info));
  if(flags & TOR_AS_PATHS)
    log_debug(LD_CIRC, "[TIP] Launching new circuit using Tor AS Paths");
 ...
 * Return the newly allocated circuit on success, or NULL on failure. */
origin_circuit_t *
as_circuit_launch_by_extend_info(uint8_t purpose, extend_info_t *extend_info, int flags, char *dest_addr)
{
  log_debug(LD_CIRC, "[TIP] Launching new circuit with exit: %s", extend_info_describe(extend_info));
  log_debug(LD_CIRC, "[TIP] Launching new circuit using Tor AS Paths");
...
//    log_fn(LOG_INFO,"%d failures so far, not trying.",n_circuit_failures);
  ...
  /* try a circ. if it fails, circuit_mark_for_close will increment
   * n_circuit_failures */
  return circuit_establish_circuit(purpose, extend_info, flags|TOR_AS_PATHS, dest_addr);
}
...
void write_circuit_info(char *filename, origin_circuit_t *circuit, entry_connection_t *connection)
{
    FILE *f = fopen(filename, "a");
    if (f == NULL)
    {
        log_debug(LD_CIRC,"[TIP] Error opening file %s. Circuit info not written to disk.", filename);
    }
    else
    {
        char *destination = connection->socks_request->address;
        char buf[200]="";
        crypt_path_t *hop;
        hop = circuit->cpath;
        do
        {
            char *temp1 = extend_info_describe(hop->extend_info);
            char *temp, split1[50], *split2, *ip;
            temp = strtok (temp1," ");
            split2 = strtok(NULL, " ");
            ip = strtok(NULL, " ");
            sprintf(split1,": %s [%s] ", ip, do_whois(ip));
            strcat(buf, split1);
            //log_debug(LD_CIRC,"[TIP] %s / %s / %s ",temp, split2, ip);
            //log_debug(LD_CIRC,"[TIP] Circuit to desitination %s has a hop at %s (AS %s)", destination, ip, do_whois(ip));
            hop = hop->next;
        }while(hop != circuit->cpath);
        log_debug(LD_CIRC,"[TIP] Buffer: %s ", buf);
        fprintf(f, "%s : %s [%s] ;\n", buf, destination, do_whois(destination));
        fclose(f);
    }
}
                                ...
                                origin_circuit_t **circp)
{
  ...
  int want_onehop;
  const or_options_t *options = get_options();
  log_debug(LD_CIRC, "[TIP] Looking for a circuit for connection to: %s. This connection has already launched %d circuits.", conn->socks_request->address, conn->num_circuits_launched);
  tor_assert(conn);
  tor_assert(circp);
  ...
  char *circuit_file = "ES-circuits-lp-1.log";
  if (circ) {
    *circp = circ;
    write_circuit_info(circuit_file, circ, conn);
  ...
  if (!circ) {
    ...
    {
      int flags = 0;//= CIRCLAUNCH_NEED_CAPACITY;
      ...
      {
        log_debug(LD_CIRC, "[TIP] No open circuits, launching a new one. Purpose: %d. Extend info: %s", new_circ_purpose, extend_info_describe(extend_info));
        circ = as_circuit_launch_by_extend_info(new_circ_purpose, extend_info, flags, conn->socks_request->address);
        log_debug(LD_CIRC, "[TIP] Launched Circuit ID: %d", circ->global_identifier);
      }
    }
    ...
    if (desired_circuit_purpose == CIRCUIT_PURPOSE_C_GENERAL) {
      ...
      if (conn->num_circuits_launched < NUM_CIRCUITS_LAUNCHED_THRESHOLD &&
        ...
        log_info(LD_CIRC, "[TIP] The application request to %s:%d has launched %d circuits without finding one it likes.", escaped_safe_str_client(conn->socks_request->address),
                    ...
                    crypt_path_t *cpath)
{
  ...
  log_debug(LD_APP|LD_CIRC, "[TIP] attaching new conn to circ. n_circ_id %u.",
